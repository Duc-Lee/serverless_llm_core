...
void run_ipc_server(hydra::VMMVirtualArena& arena) {
    ...
    listen(server_fd, 5);
    std::cout << "[Hydra Worker] UDS IPC Server listening on /tmp/hydra.sock...\n";
    ...
    while (true) {
        ...
        if (cmd.find("REMAP") != std::string::npos) {
            std::cout << "[Hydra Worker] IPC: REMAP command received. Attaching physical memory...\n";
            // arena.swap_layer_chunk(...)
        } else if (cmd.find("UNMAP") != std::string::npos) {
            std::cout << "[Hydra Worker] IPC: UNMAP command received. Scale-to-Zero (0 MB VRAM)!\n";
            // arena.unmap_all(...)
        }
...
#else
    std::cout << "[Hydra Worker] Windows OS detected. UDS fallback to stdin IPC loop...\n";
...
int main() {
    ...
    try {
        hydra::VMMVirtualArena arena(0, 80ULL * 1024 * 1024 * 1024); // 80GB VA Space
